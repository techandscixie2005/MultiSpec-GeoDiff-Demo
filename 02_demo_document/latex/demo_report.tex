...
\begin{tabular}{@{}p{0.22\textwidth} p{0.75\textwidth}@{}}
  ...
  \textbf{一键运行} &
    \texttt{git clone https://github.com/techandscixie2005/MultiSpec-GeoDiff-Demo.git \&\& cd MultiSpec-GeoDiff-Demo \&\& pip install -r 03\_code/requirements.txt \&\& python 03\_code/run\_demo.py --query\_id 0 --data 04\_data/IR\_nist\_200.jsonl --top\_k 5} \\[3pt]
  \textbf{测试命令} &
...
{\small\color{midblue}\textbf{Current implementation validates the Stage-I single-modality IR loop. Multimodal fusion, graph diffusion, pairwise-distance prediction, and TFN/SE(3)-equivariant refinement are planned extensions, not completed components.}}
...
The demo accepts a query IR spectrum, compares it against a 199-molecule NIST IR candidate library, retrieves an initial Top-M set by cosine similarity, and reranks candidates using three spectral consistency metrics (cosine similarity, L1 distance, and peak match). It reports Top-K candidates together with molecular structures, input-versus-candidate spectral overlays, coordinate-aware attention maps, and a machine-readable trace log. This workflow tests Stage-I loop feasibility under reproducible conditions; it is not a production unknown-compound identification system.
...
\begin{itemize}
  ...
  \item 未接入 Raman、NMR、MS、UV 等额外模态（当前仅 IR 单模态）
  \item 未建立生产级未知物鉴定系统
\end{itemize}
\end{scopebox}
...
Feishu 风格 Demo 说明 &
  \texttt{02\_demo\_document/DEMO说明.md} \\
Demo 说明 PDF &
...
\textbf{主命令：} \texttt{git clone https://github.com/techandscixie2005/MultiSpec-GeoDiff-Demo.git \&\& cd MultiSpec-GeoDiff-Demo \&\& pip install -r 03\_code/requirements.txt \&\& python 03\_code/run\_demo.py --query\_id 0 --data 04\_data/IR\_nist\_200.jsonl --top\_k 5} \\
