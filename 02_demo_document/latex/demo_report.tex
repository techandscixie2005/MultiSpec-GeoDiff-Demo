...
\newtcolorbox{scopebox}{
  colback=red!3!white, colframe=red!40!black, arc=3mm, boxrule=0.6pt,
  left=8pt, right=8pt, top=6pt, bottom=6pt,
  before skip=4pt, after skip=10pt
}
...
\caption{Stage-I Demo 工作流。蓝色模块为已实现的 Stage-I 管线；虚线框为未来接口模块。}
\label{fig:workflow}
\end{figure}
...
\begin{itemize}
  ...
  \item 坐标感知谱图编码、谱图距离偏置注意力、可视化输出
  \item 可追溯执行日志、22 个本地测试、GitHub Actions CI
\end{itemize}
...
\begin{itemize}
  ...
  \item 未接入 IR/Raman/NMR/MS/UV 等多模态谱图
  \item 未建立生产级未知物鉴定基准
\end{itemize}
\end{scopebox}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/fig3_roadmap.pdf}
\caption{三阶段演进路线。Stage-I（已完成为绿色）覆盖检索/重排闭环；Stage-II（计划中为琥珀色）引入图扩散与 pairwise 距离预测；Stage-III（未来为灰色）实现 SE(3) 等变细化与多模态扩展。}
\label{fig:roadmap}
\end{figure}
