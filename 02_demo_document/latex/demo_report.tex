...
Demo 以单条查询 IR 光谱为输入，在包含 200 条 NIST 实验 IR 光谱的候选库中，先由余弦相似度召回初始 Top-M 候选集，再以三项谱图一致性指标（余弦相似度、L1 距离、峰匹配）重排，最终输出 Top-K 候选分子，同时给出分子结构图、查询--候选光谱叠加图、坐标感知注意力热图及可机读 trace log。本工作流在可复现条件下验证 Stage-I 可行性闭环，定位为原理验证，并非生产级未知物鉴定系统。
