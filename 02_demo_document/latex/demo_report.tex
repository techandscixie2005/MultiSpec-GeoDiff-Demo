...
Demo 接受一条查询 IR 光谱，与包含 199 个分子的 NIST IR 候选库逐一比对，通过余弦相似度召回初始 Top-M 候选集，再由三项谱图一致性指标（余弦相似度、L1 距离和峰匹配）进行重排，最终输出 Top-K 候选分子及其分子结构图、查询与候选光谱叠加图、坐标感知注意力热图和可机读的 trace log。本工作流在可复现条件下验证 Stage-I 可行性闭环，并非生产级未知物鉴定系统。
