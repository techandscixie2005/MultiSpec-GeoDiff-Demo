...
分三阶段推进：阶段一实现多模态谱图编码与候选重排闭环（已实现）；阶段二引入大小自适应图扩散与 pairwise distance 预测；阶段三加入 parity-aware TFN-Transformer 与前向谱图一致性验证。数据来源包括 QM9S、NIST IR 及公开谱图数据库。主要风险为实验噪声、一谱多解歧义与数据覆盖不足；验证指标包括 Top-K 命中率、谱图相似度、距离 MAE、三维 RMSD 及重排增益。
